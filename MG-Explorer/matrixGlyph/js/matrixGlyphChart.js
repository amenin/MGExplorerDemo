...
define(["model","libCava","numericGlyph"], function (Model,LibCava,NumericGlyph) {
  return function MatrixGlyph (idDiv) {
    ...
    var _matrixGlyphPanel = null,  // Representa o panel associado aao grafico
        ...
        _grpBarsLL = null,    // Seleção com todos os grupos das barras da legenda esquerda
        _grpBarsTL = null,
        _grpLines  = null,    // Contém as linhas com células em cada uma
		
	    _legend = {              // Barra onde estão os textos como os nomes dos nodos
           margin : 1,          // Distância do retângulo da legenda até a matrix 
           marginText : 1,      // Distância do texto até o retângulo
		...
		_cell = {
		   height : 0,           // Altura/largura da célula (calculado)
		...
		_cellColorMap = {
		   colorScale : _cellColorScaleDefault,     // Escala de cores usada no mapeamento da cor de fundo das células
		   indexAttrColor : 0,                        // Índice do atributo que será mapeado para cor
		...
		_lines = {
		  ...
		  qtMaxVisible : 0,  // Quantidade máxima de linhas visiveis (calculado)
		  qtMinVisible : 0,  // Quantidade mínima de linhas visiveis (calculado)
		  qtTotal: 0        // Quantidade total de linhas da matriz (calculado)
		},
		
		_tooltips = {
			divTT: null, // Div onde o toolTip será inserido
			matrixCell: null,   // Objeto que gerencia o tooltip para a célula
			xCell: -1,    // Coluna da célula onde está o tooltip. -1 Se não está ativo
			yCell: -1    // Coluna da célula onde está o tooltip. -1 Se não está ativo			
		},		
			
		_matrixWidth = 0,       // Largura da área da matriz (calculado)
		_matrixHeight = 0,      // Altura da área da matriz (calculado)
			
        _idClipLeft = idDiv + "l",    // Id da área de recorte da legenda esquerda
        _idClipTop  = idDiv + "t",    // Id da área de recorte da legenda do topo
        _idClipMatrix = idDiv + "m",  // Id da área de recorte da matriz
	
		_indexAttrSort = 0,  // Índice do atributo utilizado para o sort (0-primeiro labels[] 1000-primeiro values[])
		_indexAttrLegend = 0,  // Índice do atributo que será impresso na legenda (nodo)
		_vOrder = null,      // Vetor indireto de ordenacao
		
		_cellCoordScale = d3.scale.ordinal(),    // Escala usada para definir as coordenadas das células e elementos da legenda
...
// ---------------- Atributos geométricos do grafico	
		...
		model.pLegendWidth = 0.15;  // Percentual da largura em relação ao model.widthChart
		model.pFontHeight = 0.0225;   // Percentual da altura da fonte em relaçã ao model.widthChart
...
//===================================================
  ...
  //--------------------- 
    model.when(["data","widthChart","heightChart","redraw"], function (data,widthChart,heightChart, redraw ) {
	  ...
	  _matrixGlyphPanel.update();  // Atualiza informações no painel associado a técnica	  
...
//--------------------------------- Funcoes privadas
    ...
    function _appendLeftLegend (data) {
      ...
      _grpBarsLL.append("text")
			.attr("class", "GM-node")		  
			.attr("x", _legend.marginText)       // -3 margem do texto em relação ao retângulo onde ele está inserido            
	  ...
	  _grpLines = _grpDragMT.selectAll("g")
		...
		.enter()
			.append("g")
				.attr("class", "MG-Line")
				.attr("transform", function(d, i) { return "translate(0," + _cellCoordScale(i) + ")"; })
				.each( function (d,i) {          // this é a referência para cada elemento da seleção .line
					drawCells(d,this);    // Desenha cada célula da matriz
				});

      function drawCells(d,elemThis) {
	  	var cells = d3.select(elemThis).selectAll(".MG-Cell")
			.data( d.filter(function(d) { return d.exist; }))  // Seleciona as células que possuem dados
			.enter()
				.append("g")
					.attr("class", "MG-Cell")
					.attr("transform", function(d, i) { return "translate(" + _cellCoordScale(d.x) +",0)"; }) // Inclui grupos de cada célula da linha
 ...
 * _calcHeightCell
 * 
 * Calcula a altura/largura da célula. É a mesma para as barras da legenda
 */
    function _calcHeightCell () {
	  var h = _cellCoordScale.rangeBand();
      if ( h > 2)
  	     h -= 1;  // Ajusta para margem de 1 px entre células
      return h;	
    };	
