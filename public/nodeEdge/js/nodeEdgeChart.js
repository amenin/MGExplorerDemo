...
define(["model", "algCluster"], function (Model, AlgCluster) {
    return function NodeEdge(idDiv) {
            ...
            if (_graphData.nodes[0].cluster) {
                ...
                _graphElem.nodes = _grpNodeEdge.selectAll(".NE-node")
                    ...
                    .style("fill", function (d) {
                        ...
                        return _colorScale(d.values[12])
                    })
        ...
        chart.getColorBreaks = function() {
            let breaks = model.data.nodes.dataNodes.map(d => d.values[12])
            breaks = breaks.filter((d,i) => breaks.indexOf(d) == i).sort()
            return breaks;
        };

        chart.getColorScale = function() {
            return _colorScale;
        };

        chart.getColorLabels = function(){
            return _colorLabels;
        }
